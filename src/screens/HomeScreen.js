...
function AddExpenseModal({ visible, onClose, onAdd }) {
  ...
  const handleAdd = () => {
    ...
    if (!amount || isNaN(amount)){ Alert.alert('Missing', 'Please enter a valid amount.'); return; }
    if (!members || isNaN(members) || parseInt(members) < 2) { Alert.alert('Invalid', 'Members must be 2 or more.'); return; }
    ...
    setTitle(''); setAmount(''); setMembers('2'); setLocation(''); setEmoji('🍽️'); setColorIdx(0);
    onClose();
  };
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <KeyboardAvoidingView style={{ flex:1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <View style={m.sheet}>
          ...
          <View style={m.handle} />
          <Text style={m.sheetTitle}>Add Expense</Text>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 14, paddingBottom: 20 }}>
            ...
            <View style={m.inputWrap}>
              <View style={m.inputBase} />
              <TextInput style={m.input} placeholder="e.g. Dinner at Spice Route" placeholderTextColor="rgba(255,255,255,0.3)" value={title} onChangeText={setTitle} />
            </View>

            <Text style={m.label}>Total Amount (₹) *</Text>
            <View style={m.inputWrap}>
              <View style={m.inputBase} />
              <TextInput style={m.input} placeholder="0" placeholderTextColor="rgba(255,255,255,0.3)" value={amount} onChangeText={setAmount} keyboardType="numeric" />
            </View>

            <Text style={m.label}>Number of People *</Text>
            <View style={m.inputWrap}>
              <View style={m.inputBase} />
              <TextInput style={m.input} placeholder="2" placeholderTextColor="rgba(255,255,255,0.3)" value={members} onChangeText={setMembers} keyboardType="numeric" />
            </View>

            <Text style={m.label}>Location (optional)</Text>
            <View style={m.inputWrap}>
              <View style={m.inputBase} />
              <TextInput style={m.input} placeholder="e.g. Connaught Place, Delhi" placeholderTextColor="rgba(255,255,255,0.3)" value={location} onChangeText={setLocation} />
            </View>

            {/* Per person preview */}
            {amount && members && !isNaN(amount) && !isNaN(members) && parseInt(members) > 0 && (
              <View style={m.previewRow}>
                <LinearGradient colors={COLOR_OPTIONS[colorIdx]} style={m.previewPill} start={{x:0,y:0}} end={{x:1,y:0}}>
                  <Text style={m.previewText}>
                    ₹{Math.round(parseFloat(amount) / parseInt(members)).toLocaleString('en-IN')} per person
                  </Text>
                </LinearGradient>
              </View>
            )}
...
function BalanceCard({ expenses }) {
  ...
  const perYou  = expenses.reduce((s, e) => s + Math.round(e.amount / e.members), 0);
  const totalOwed = Math.round(total * 0.6);
    ...
    <View style={s.heroOuter}>
      <LinearGradient colors={['#2A1570','#16085E','#0A0525']} style={s.heroCard} start={{x:0,y:0}} end={{x:1,y:1}}>
        ...
        <View style={s.heroContent}>
          ...
          <Text style={s.heroSub}>{expenses.length} splits tracked</Text>
        </View>
        <View style={s.statsRow}>
          ...
          <View style={s.statBox}>
            ...
            <AnimatedNumber value={perYou} style={[s.statValue, { color:'#FF4D6D' }]} />
          </View>
        </View>
      </LinearGradient>
    </View>
...
export default function HomeScreen() {
  ...
  const avatar = profile?.avatar ?? (profile?.name?.charAt(0).toUpperCase() ?? '?');
      ...
      >
        ...
        <View style={s.sectionHeader}>
          <Text style={s.sectionTitle}>
            {expenses.length === 0 ? 'No Splits Yet' : 'Recent Splits'}
          </Text>
          {expenses.length > 0 && (
            <Text style={s.splitCount}>{expenses.length} total</Text>
          )}
        </View>

        {expenses.length === 0 ? (
          <View style={s.emptyState}>
            <View style={s.emptyBase} />
            <BlurView intensity={20} tint="dark" style={[StyleSheet.absoluteFillObject,{borderRadius:20}]} />
            <View style={s.emptyBorder} />
            <Text style={s.emptyEmoji}>💸</Text>
            <Text style={s.emptyTitle}>No expenses yet</Text>
            <Text style={s.emptySub}>Tap "+ Add" to track your first split</Text>
          ...
          <View style={s.splitList}>
            {expenses.map((item, i) => (
              <SplitCard key={item.id} item={item} index={i} onDelete={handleDelete} />
            ))}
          </View>
        )}
      </ScrollView>

      <AddExpenseModal visible={showModal} onClose={() => setShowModal(false)} onAdd={handleAdd} />
    </MeshBackground>
  );
}
...
const s = StyleSheet.create({
  ...
  splitCount:    { color:'rgba(255,255,255,0.40)', fontSize:13 },

  emptyState: { borderRadius:20, overflow:'hidden', alignItems:'center', paddingVertical:40, paddingHorizontal:24, gap:10 },
  emptyBase:  { ...StyleSheet.absoluteFillObject, borderRadius:20, backgroundColor:'rgba(18,10,50,0.85)' },
  emptyBorder:{ ...StyleSheet.absoluteFillObject, borderRadius:20, borderWidth:1, borderColor:'rgba(255,255,255,0.12)' },
...
const m = StyleSheet.create({
  ...
  sheetTitle: { color:'#FFFFFF', fontSize:22, fontWeight:'800', letterSpacing:-0.3, marginBottom:20 },
  ...
  addBtn:     { height:54, borderRadius:16, alignItems:'center', justifyContent:'center', shadowColor:'#7B61FF', shadowOffset:{width:0,height:6}, shadowOpacity:0.5, shadowRadius:14, elevation:10 },
